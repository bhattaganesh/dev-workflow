 ...
 * Values can be a string or string[] — arrays return the first path that exists
 * on disk, enabling transparent fallback across install locations.
 */
export function getPath(paths = {}) {
  const val = paths[platform] ?? paths.linux ?? '';
  if (Array.isArray(val)) return val.find((p) => existsSync(p)) ?? '';
  return val;
}
